...
async function createController(req, res) {
    console.log(req.file, req.body, "www")
    const { title, type, img, curse, codigo } = req.body;
    if (req.file) {
        const newCurse = { title, type, img: req.file.filename, curse, codigo };
...
async function deleteController(req, res) {
    ...
    const note = await Filecurse_1.default.findById(req.params.id);
    const file = note.img

    // const curses = await _2_Curse_1.default.findById(id);
    if (note) {
        try {
            await fs_extra_1.default.unlink(path_1.default.resolve(file));
        }
        catch (err) {
            console.error(err);
        }
