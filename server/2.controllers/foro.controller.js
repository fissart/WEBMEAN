...
const land_1 = __importDefault(require("../1.models/foro"));
//createController///////////////////////////////////////////////////////////////////////
async function createController(req, res) {
    console.log(req.body);
    // const { title, type, img, curse, codigo, foreign, description } = req.body;
    // if (req.file) {
...
async function getsController(req, res) {
    const data = await land_1.default.aggregate([
        {
            $match: {
                type: "theme",
            },
                ...
                pipeline: [
                    { $match: { $expr: { $eq: ["$foreign", "$$www"] } } },
                    {
                        $lookup: {
                            ...
                            let: { w_www: "$_id" },
                            pipeline: [
                                { $match: { $expr: { $eq: ["$foreign", "$$w_www"] } } },
                                {
                                    $lookup: {
                                        from: "users",
                                        let: { userrr: "$user" },
                                        pipeline: [
                                            { $match: { $expr: { $eq: ["$_id", "$$userrr"] } } }],
                                        as: "usser",
                                    },
                                },
                            ],
