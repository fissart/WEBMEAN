...
async function getSController(req, res) {
    ...
    var year = "2025"//new Date().getFullYear();
...
async function getConvalidation(req, res) {
    console.log(req.body)
    const campo=req.body.campo
    const Usser = await _1_User_1.default.find({ [campo]: { '$regex': new RegExp(req.body.ww, 'i') } }).collation({ locale: 'es' })
    // console.log(req.body.ww, Usser)
    return res.json({
        Usser
    })
}
exports.getConvalidation = getConvalidation
...
async function getupdateController(req, res) {
    ...
    jwt.verify(token, 'ESFA', async (err, decoded) => {
        if (err) {
            console.log(token)
            return res.status(401).json({
                error: "Tokenwww expirado",
            });
        }
...
async function signin(req, res) {
    ...
    if (user.password !== uu) // return res.status(401).send('Wrong Password');
        return res.json({ user: { "msg": "Password incorrecto" } });
    const token = jwt.sign({ _id: user._id, }, 'ESFA', { expiresIn: "359m", })
    return res.json({ token, user });
}
