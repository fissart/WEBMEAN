...
export async function getcursesources(req: Request, res: Response): Promise<Response> {
    ...
    if (month <= 6) {
        const Curses = await Cursesource.find({ ciclo: { $in: ['I', 'III', 'V', 'VII', 'IX'] }, mencion: { $in: ['ED', 'G', 'P', 'E'] } })
        return res.json(Curses);
    } else {
        const Curses = await Cursesource.find({ ciclo: { $in: ['II', 'IV', 'VI', 'VIII', 'X'] }, mencion: { $in: ['ED', 'G', 'P', 'E'] } })
        return res.json(Curses);
    }
    //return res.json("Curses");
};
