...
export async function createController(req: Request, res: Response): Promise<Response> {
    const {note, task, dateb, datee, solution, theme, unidad, curse, user, asistence } = req.body;
    //console.log(patth);
    if (req.file) {
      const patth=req.file.path;
      const newCurse = { note, img: patth, task, dateb, datee, solution, theme, unidad, curse, user, asistence };
      const userw = new Task(newCurse);
      await userw.save();
    } else {
      const newCurse = { note, task,  dateb, datee, solution, theme, unidad, curse, user, asistence };
