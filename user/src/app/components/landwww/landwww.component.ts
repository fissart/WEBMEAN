...
// import { UsersServicewww } from '../../services/users.service copy'
...
export class LandwwwComponent implements OnInit {
  ...
  public conf = { language: 'es', toolbar: [], isReadOnly: 'true' }
  //  public configg = { language: 'es', toolbar: ["heading", "|", "bold", "italic", "link", "bulletedList", "numberedList", "|", "indent", "outdent", "|", "blockQuote", "insertTable", "math", "mediaEmbed", "|", "undo", "redo"] }
  ...
  constructor(
    ...
    // private chatService: UsersServicewww,
    private router: Router,
    private UserService: UsersService,
      // private www: TimeagoIntl,
    private Tw: Title,
    private modal: NgbModal,
    private iconRegistry: MatIconRegistry,
    private sanitizer: DomSanitizer
  ) {
    // www.strings = englishStrings;
    // www.changes.next();
...
`));
  ...
  //   this.chatService.sendMessage("www")//.subscribe((message: string) => {
  // };
  ...
  ngOnInit() {
    ...
    // console.log(this.chatService.ioSocket.connected)
    // this.chatService.on('connect', () => {
    //   console.log('Connected', this.chatService.ioSocket.id);
    // });
    this.getforo()
    // this.getforo()
  ...
  openwww(w: any, title: string, description: string, img: string) {
    ...
    this.text = title;
    this.textoimg = img;
    this.texto = description//.replace(new RegExp('</p>', 'g'), '').replace(new RegExp('<p>', 'g'), '')      .replace(/(<script type="math\/tex; mode=display">)(.*)(<\/script>)/g, '$$$$$2$$$$')      .replace(/<figure class="media">/g, '').replace(/<\/figure>/g, '').replace(/(<oembed url="https:\/\/www.dailymotion.com\/video\/)(.*?)("><\/oembed>)/g, `<iframe width='100%' height='350' src="https://www.dailymotion.com/embed/video/$2"></iframe>`)      .replace(/(<script type="math\/tex">)(.*)(<\/script>)/g, '$$$2$$').replace(/(<oembed url="https:\/\/www.youtube.com\/watch\?v=)(.*?)(".*?oembed>|&.*?oembed>)/g, `<iframe width='100%' height='350' src="https://www.youtube.com/embed/$2"></iframe>`)    this.textoimg = img;
  }
  ...
  open2forum(forum: any, idnew: string, title: string, description: string, type: string, proceso: string) {
    if (localStorage.getItem('id')) {
      ...
      console.log(idnew, title, description, proceso, type);
      this.idnew = idnew;
      this.titlewww = title;
          ...
          const arrayReverseObj = (res: any) => {
            ...
            for (const key in res) {
              if (Object.prototype.hasOwnProperty.call(res, key)) {
                // Example: Prefix string values with "Updated: "
                // console.log(res[key].description)
  ...
  createThemeforo() {
    if (this.titlewww != '' && this.markdown != '') {
      ...
      this.CurseService.foronew(this.user || "", this.idnew == 'www' ? this.user || "" : this.idnew, this.titlewww, this.type, this.markdown, this.markdown
  ...
  updateThemeforo() {
    if (this.titlewww != '' && this.markdown != '') {
      this.loading = "false"
      console.log(this.idnew, this.titlewww, this.markdown)
      this.CurseService.themeforoupdate(this.idnew, this.titlewww, this.markdown, this.markdown
        //.replace(new RegExp('</p>', 'g'), '').replace(new RegExp('<p>', 'g'), '').replace(/(<script type="math\/tex; mode=display">)(.*)(<\/script>)/g, '$$$$$2$$$$').replace(/<figure class="media">/g, '').replace(/<\/figure>/g, '').replace(/(<oembed url="https:\/\/www.dailymotion.com\/video\/)(.*?)("><\/oembed>)/g, `<iframe width='100%' height='350' src="https://www.dailymotion.com/embed/video/$2"></iframe>`)        .replace(/(<script type="math\/tex">)(.*)(<\/script>)/g, '$$$2$$').replace(/(<oembed url="https:\/\/www.youtube.com\/watch\?v=)(.*?)(".*?oembed>|&.*?oembed>)/g, `<iframe width='100%' height='350' src="https://www.youtube.com/embed/$2"></iframe>`)
      )
        .subscribe((res: any) => {
          this.getforo()
