...
export class LandwwwService {
  ...
  foronew(user: string, foreign:string, title: string, type: string, description: string, descriptionnew: string) {
    return this.http.post(`${environment.apiURL}/api/foro/`, {user,foreign, title,type,description, descriptionnew});
  }
  getforum() {
    return this.http.get(`${environment.apiURL}/api/foro`);
  }
  themeforoupdate(id: string, title: string, description: string, descriptionnew: string) {
    return this.http.put(`${environment.apiURL}/api/foro/${id}`, { title, description, descriptionnew });
  }
  themeforodelete(id: string) {
    return this.http.delete(`${environment.apiURL}/api/foro/${id}`);
  }
  ...
  */
  ...
  newscreate(iduser: string) {
    return this.http.post(`${environment.apiURL}/api/news/Controller`, {
      'user': iduser, "title": "Cambiar título", "description": `<h2>Título 1</h2><h3>Título 2</h3><h4>Título 3</h4><p>www www</p><figure class="image"><img src="https://thumbs.dreamstime.com/b/elegant-blue-background-swirls-space-your-text-elegant-blue-background-swirls-space-your-text-159291073.jpg"></figure><ul><li><a href="https://drive.google.com/file/u/0/d/0B1Ga40SmVyy-TTRWcXNkRGYxY00/view?resourcekey=0-tXQ867e7G7OY2bMQzshTKQ">Libro</a></li><li><a href="https://marcianosmx.com/10-fractales-alucinantes-puedes-encontrar-la-naturaleza/">Sucesiones en la naturaleza</a></li></ul><script type="math/tex; mode=display">\\sum_2^3\\int_1^2</script> `, "show": false
    });
  }
