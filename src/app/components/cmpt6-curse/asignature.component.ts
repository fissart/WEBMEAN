...
export class AsignatureComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.Scn = scn
        console.log(format1, str, "kkkkkkkk")
    ...
    savetask(task: string, dateb: string, datee: string, respuesta: string, idtheme: string, idunity: string, idcurse: string, iduser: string,) {
        if (task != '' && respuesta != '' && idtheme != '' && idunity != '' && idcurse != '' && iduser) {
            this.taskk.savetask('', task, dateb, datee, respuesta, idtheme, idunity, idcurse, iduser, 'P', this.archivos[0])
                .subscribe(
                    (res: any) => {
                        this.loading = "false";
